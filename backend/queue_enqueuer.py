"""
backend/queue_enqueuer.py — Enqueues PR review jobs into Redis via ARQ.
...
def _enqueue_sync(function_name: str, job_id: str, payload: dict) -> str:
    ...
    # Serialise with ARQ's own function (defaults to pickle)
    job_bytes = serialize_job(
        ...
        serializer=None,  # use default (pickle)
    )
...
async def init_pool() -> None:
    ...
    print("Queue enqueuer: Redis connection OK")
...
async def enqueue_review_job(payload_bytes: bytes, delivery_id: str) -> str:
    ...
    print(f"[Enqueuer] Job {job_id} enqueued to Redis queue '{ARQ_QUEUE}'")
    return job_id
